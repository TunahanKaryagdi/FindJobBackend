...
{
    public class GetApplicationsByUserIdQuery : IRequest<IDataResult<List<ApplicationDto>>>
    {
        ...
        public class GetApplicationsByUserIdQueryHandler : IRequestHandler<GetApplicationsByUserIdQuery, IDataResult<List<ApplicationDto>>> 
        {
            ...
            public GetApplicationsByUserIdQueryHandler(IApplicationReadRepository applicationReadRepository)
            {
                _applicationReadRepository = applicationReadRepository;
            }


            public async Task<IDataResult<List<ApplicationDto>>> Handle(GetApplicationsByUserIdQuery request, CancellationToken cancellationToken)
            {
                List<ApplicationDto> applications = _applicationReadRepository.GetWhere(a => a.UserId == Guid.Parse(request.UserId)).Select(a =>
